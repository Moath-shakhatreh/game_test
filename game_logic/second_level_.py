...
back_ground_level_2 = Back_ground_scale(SCREEN_WIDTH,SCREEN_HEIGHT)
		
#######################
		
class Image():
	def __init__(self,direction,flip = False):
		self.direction = direction
		self.flip = flip

	def importing(self):
		if self.direction == "idel":
			image = []
			for i in range(1,7):
				image_ = pygame.image.load(f"assets\\character_animation\\blue\\idle\\idle{i}.png").convert_alpha()
				image_ = pygame.transform.flip(image_,self.flip,False)
				image.append(pygame.transform.scale2x(image_))
		elif self.direction == "running":
			image = []
			for i in range(1,9):
...
class Obstacle(pygame.sprite.Sprite):
    ...
    @classmethod
    def collide(self):
        if self.rect.colliderect(player_level_2.sprite.rect): 
...
class Monsters(pygame.sprite.Sprite):
    ...
    def collide(self):
        current_time = pygame.time.get_ticks()
        if self.rect.colliderect(player_level_2.sprite.rect) and not(key[pygame.K_RSHIFT]) and current_time - player_level_2.sprite.last_update >= player_level_2.sprite.hit_cooldown:
...
monsters_level_2.add(Monsters('wispy',objects_level_2.sprites()[2].rect.x  ,objects_level_2.sprites()[1].rect.top,1000,5,2))
